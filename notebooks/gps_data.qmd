...
# Beitbridge Truck GPS

-   This notebook is for checking data quality on the GPS data

-   Subsetting for our Study Period

-   Adding zeros to dates where no data was collected
...
```{r}
#fix column names
gps_data = gps_data %>% rename_with(~ str_remove(., "Border_Crossing_"), everything())
...
-   Start_hour - not consistent need to add zeros to count events and other fields
